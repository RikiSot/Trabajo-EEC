...
typedef struct{
	union{
		...
		struct fields{
			char name[8];
			char extension[3];
			unsigned int8 attrib;			///sólo lectura, oculto, de sistema, directorio...
			//unsigned int8 reserved[10];
			  ...
			  unsigned int32 size;			///tamaño en bytes
		};
	};
} fileEntry;
...
typedef struct{
	...
	unsigned int16 posInSector;      //posición actual al escribir en el sector
	unsigned int32 sectorAtFAT;      //sector de la 1ª FAT en el que tenemos el actual puntero de cierre de fichero
	
	unsigned int32 sectorEntryFile; //dirección del sector de entradas de fichero
	unsigned int8  EntryFile[512];  //sector donde está la entrada (root) del fichero (para actualizar tamaño)
	unsigned int16 offsetEntry;     //offset en el sector de entradas del fichero
	unsigned int32 size;            //tamaño del fichero
}_file;
...
#define getDate() (((int16)year - 1980) << 9 ) + ((int16)month << 5) + (int16)day	//7 bits: año (desde 1980), 4 bits: mes, 5 bits: dia
...
unsigned int8 inicializa_fichero(){
   ...
   unsigned int32 startCluster;      //cluster donde empezarían los datos del fichero
   fileEntry newFile;
   ...
   for(i = SctStart; i < SctEnd; i++){			      
		  ...
		  for(j = 0; j < 512; j += 32){
			if((dt[j] == 0x00) || dt[j] == 0xFF){
					   newFile.fields.name[0] = 		'F';
					   newFile.fields.name[1] = 		'I';
					   newFile.fields.name[2] = 		'C';
					   newFile.fields.name[3] = 		'H';
					   newFile.fields.name[4] = 		'E';
					   newFile.fields.name[5] = 		'R';
					   newFile.fields.name[6] = 		'O';
					   ...
					   newFile.fields.attrib = __FILE | READONLY;   //archivo normal + sólo lectura
					   ...
					   if(FATType==16){										
					   startCluster = encontrarCluster(2);         											//busco cluster donde empezaría a almacenarse los datos
					   file.currentCluster = startCluster;   												//que será el cluster en el que escribiremos
					   ...
					   if(FATType==32){										
					   startCluster = encontrarCluster(3);         											//busco cluster donde empezaría a almacenarse los datos
					   file.currentCluster = startCluster;   												//que será el cluster en el que escribiremos
					   ...
					   file.size = 0;																			//Inicialmente el tamaño es cero
...
void escribe_datos_en_fichero(){
   ...
   unsigned int16 _hour, date;
	...
	sd_write_block(file.currentSector, dt); 
	    ...
	    if((relSector % sect_x_cluster) == 0){   											//¿el siguiente sector está en cluster nuevo?
	   ...
	   //actualizar el tamaño del fichero y fechas
	   file.size += 512;
	   file.EntryFile[file.offsetEntry + 28] = make8(file.size, 0);   //tamaño
	   ...
	   file.EntryFile[file.offsetEntry + 18] = file.EntryFile[file.offsetEntry + 24];   //fecha último acceso
